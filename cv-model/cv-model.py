...
# Tip Ids of all the fingers
tipIds = [4, 8, 12, 16, 20]

# Game state and position variables
game_started = 1
charac_pos = [0,1,0]
index_pos = 1
fixedx = None
...
# Initializing hand detection with confidence levels
with mp_hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
    # Initializing holistic (whole body) detection with confidence levels
    with mp_holistic.Holistic(min_detection_confidence=0.5,min_tracking_confidence=0.5) as holistic:
        
        # Loop to continuously capture frames from video
        while True:
            ...
            # Process the image for holistic and hand detection
            results_holistic = holistic.process(img)
            results_hands = hands.process(img)
            img = cv2.cvtColor(img,cv2.COLOR_RGB2BGR)
            
            # Getting center coordinates of the frame
            width_hf = int(width/2)
            height_hf = int(height/2)

            # Checking if pose landmarks are detected (body keypoints)
            if results_holistic.pose_landmarks:
                ...
                # Getting the midpoint between the shoulders
            ...
            # Lists to store hand landmarks and finger states
            hand_cor_list_right = []
            hand_cor_list_left = []
            hand_type1 = None
            hand_type2 = None
